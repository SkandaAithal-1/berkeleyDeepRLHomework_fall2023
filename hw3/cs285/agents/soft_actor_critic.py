...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # NOTE: We might have to do reward expand 
            target_values: torch.Tensor = reward + self.discount * next_qs * (1 - 1.0*done)
        ...
        # Compute loss
        loss: torch.Tensor = self.critic_loss(q_values, target_values.float())
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action_samples = action_distribution.rsample((1000,))
        entropy = -torch.mean(action_distribution.log_prob(action_samples), dim=0)
        return entropy
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # NOTE: Might have to reshape action
